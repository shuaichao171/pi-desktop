...
function AutomationEditor({ initial, workspaces, models, busy, error, onSave, onClose }: { initial: UiAutomationInput; workspaces: string[]; models: UiModelSummary[]; busy: boolean; error: string | null; onSave(input: UiAutomationInput): void; onClose(): void }) {
	...
	const [grace, setGrace] = useState(initial.misfireGraceMinutes == null ? '' : String(initial.misfireGraceMinutes));
	const [runLimit, setRunLimit] = useState(initial.maxScheduledRuns == null ? '' : String(initial.maxScheduledRuns));
	const [retries, setRetries] = useState(String(initial.dispatchRetryLimit ?? 2));
	...
	const draftSchedule: UiAutomationSchedule | null = frequency === 'interval'
		? Number.isInteger(Number(minutes)) && Number(minutes) >= 5 && Number(minutes) <= 43200 ? { kind: 'interval', minutes: Number(minutes) } : null
		: frequency === 'once' ? Number.isFinite(new Date(onceAt).getTime()) ? { kind: 'once', at: new Date(onceAt).toISOString() } : null
		: time && (frequency !== 'weekly' || days.length) ? { kind: 'weekly', time, days: frequency === 'daily' ? DAYS : frequency === 'weekdays' ? WEEKDAYS : days } : null;
	const scheduleZone = frequency === 'once' ? localZone() : zone;
	function submit(event: FormEvent) {
		...
		onSave({ ...initial, name: name.trim(), prompt: prompt.trim(), cwd, model: modelKey ? { provider, id: modelId } : null, thinkingLevel: selectedModel && !selectedModel.reasoning ? null : thinking || null, schedule, timeZone, enabled, misfireGraceMinutes: grace ? Number(grace) : null, maxScheduledRuns: runLimit ? Number(runLimit) : null, dispatchRetryLimit: Number(retries) });
	}
	return <Modal title={t(initial.id ? 'automation.edit' : 'automation.create')} onClose={onClose} busy={busy}>
		<form onSubmit={submit} className="pd-automation-form">
			<div className="pd-automation-form-body">
				<section className="pd-automation-editor-section" aria-label={copy.taskSection}><h3>{copy.taskSection}</h3>
				...
				</section><section className="pd-automation-editor-section" aria-label={copy.projectSection}><h3>{copy.projectSection}</h3>
				...
				<details className="pd-automation-model-options" open={Boolean(initial.model || initial.thinkingLevel)}><summary>{copy.options}<span>{selectedModel?.name || (modelKey ? initial.model?.id : t('automation.defaultModel'))} · {thinking ? t(`composer.thinking.${thinking}`) : t('automation.defaultThinking')}</span></summary><div className="pd-automation-field-pair">
				...
				<section className="pd-automation-editor-section pd-automation-schedule-fields" aria-label={copy.timeSection}><h3>{copy.timeSection}</h3>
					<p className="pd-automation-schedule-summary" aria-live="polite"><strong>{copy.plan}</strong><span>{draftSchedule ? scheduleLabel(draftSchedule, locale, t, scheduleZone) : copy.invalidPlan}</span><span>{t('automation.timeZone')}: {scheduleZone || '—'}</span></p>
					...
					<div className="pd-automation-field-pair"><label className="pd-automation-field"><span>{t('automation.grace')}</span><input type="number" min={1} max={525600} value={grace} onChange={event => setGrace(event.target.value)} disabled={busy} placeholder={t('automation.unlimited')} /><small>{t('automation.graceHint')}</small></label><label className="pd-automation-field"><span>{t('automation.runLimit')}</span><input type="number" min={1} max={100000} value={runLimit} onChange={event => setRunLimit(event.target.value)} disabled={busy} placeholder={t('automation.unlimited')} /><small>{t('automation.runLimitHint')}</small></label></div>
					<label className="pd-automation-field"><span>{t('automation.dispatchRetries')}</span><input type="number" required min={0} max={5} value={retries} onChange={event => setRetries(event.target.value)} disabled={busy} /><small>{t('automation.dispatchRetriesHint')}</small></label>
				</section>
...
export function AutomationPage({ onToggleSidebar, onOpenSession, headerControls }: { onToggleSidebar(): void; onOpenSession(cwd: string, path: string): Promise<boolean>; headerControls?: ReactNode }) {
	...
	const [historyFilter, setHistoryFilter] = useState<UiAutomationRun['status'] | 'all'>('all');
	...
	const history = automationRuns(snapshot.runs, historyId ?? 'all', historyFilter);
	function showHistory(id: string, runId?: string) { setHistoryFilter('all'); setSelectedRunId(runId ?? null); setHistoryId(id); }
	useEffect(() => {
		if (!historyId || !selectedRunId) return;
		const row = [...(historyBody.current?.querySelectorAll<HTMLElement>('[data-run-id]') ?? [])].find((entry) => entry.dataset.runId === selectedRunId);
		row?.scrollIntoView({ block: 'center' }); row?.focus({ preventScroll: true });
	}, [historyId, selectedRunId]);
		...
		<div className="pd-automation-scroll"><div className="pd-automation-content">
			...
			{loading ? <div className="pd-automation-empty" role="status">{t('automation.loading')}</div> : <>
				{!visible.length ? <div className="pd-automation-empty"><span className="pd-automation-empty-icon"><Icon name={snapshot.automations.length ? 'search' : 'automation'} width="26" height="26" /></span><h2>{t(snapshot.automations.length ? 'automation.noMatches' : 'automation.emptyTitle')}</h2><p>{t(snapshot.automations.length ? 'automation.noMatchesHint' : 'automation.emptyHint')}</p>{!snapshot.automations.length && <button type="button" className="pd-automation-button" onClick={() => create()} disabled={!bridge || busy}>{t('automation.createFirst')}<Icon name="arrowRight" width="15" height="15" /></button>}</div> : <div className="pd-automation-grid">{visible.map((task) => {
					const taskRuns = automationRuns(snapshot.runs, task.id);
					const run = taskRuns.find((entry) => (entry.status === 'running' || entry.status === 'retrying'));
					const latest = taskRuns.find((entry) => (entry.status !== 'running' && entry.status !== 'retrying'));
					return <article className={`pd-automation-card${run ? ' is-running' : ''}`} key={task.id}><div className="pd-automation-card-heading"><span className="pd-automation-card-icon"><Icon name="automation" /></span><span className={`pd-automation-status is-${task.enabled ? 'active' : 'paused'}`}><i />{t(task.completedAt ? 'automation.completed' : task.enabled ? 'automation.active' : 'automation.paused')}</span><button type="button" className="pd-automation-button is-quiet" onClick={() => { setEditorError(null); setEditor(task); }} disabled={busy}>{t('automation.edit')}</button></div><button type="button" className="pd-automation-card-title" onClick={() => showHistory(task.id)}><h2>{task.name}</h2><Icon name="chevronRight" width="16" height="16" /></button><p className="pd-automation-prompt-preview">{task.prompt}</p>{run && <span className="pd-automation-status is-running"><i />{t(`automation.status.${run.status}`)}</span>}{latest ? <button type="button" className={`pd-automation-latest is-${latest.status}`} onClick={() => showHistory(task.id, latest.id)}><span>{copy.latestRun} · {t(`automation.status.${latest.status}`)}</span><time dateTime={latest.finishedAt || latest.startedAt}>{dateLabel(latest.finishedAt || latest.startedAt, locale, task.timeZone)}</time><Icon name="chevronRight" width="14" height="14" /></button> : !run && <p className="pd-automation-never-run">{copy.neverRun}</p>}<div className="pd-automation-card-meta"><span title={task.cwd}><Icon name="folder" width="14" height="14" />{pathLeaf(task.cwd)}</span><span title={task.timeZone}><Icon name="clock" width="14" height="14" />{scheduleLabel(task.schedule, locale, t, task.timeZone)}</span></div><p className="pd-automation-next">{t('automation.count', { count: task.scheduledRunCount ?? 0 })}{task.maxScheduledRuns ? ` / ${task.maxScheduledRuns}` : ''}</p><p className="pd-automation-next">{task.completedAt ? t('automation.completedHint') : task.enabled && task.nextRunAt ? t('automation.next', { time: dateLabel(task.nextRunAt, locale, task.timeZone), zone: task.timeZone }) : t(task.enabled ? 'automation.noNext' : 'automation.pausedHint')}</p><footer><button type="button" className="pd-automation-button" disabled={busy || !bridge} onClick={() => bridge && void perform(() => run ? bridge.cancelAutomationRun(run.id) : bridge.runAutomation(task.id))}><Icon name={run ? 'square' : 'arrowRight'} width="13" height="13" />{t(run ? 'automation.stop' : 'automation.run')}</button><button type="button" className="pd-automation-button is-quiet" disabled={busy || !bridge || Boolean(task.completedAt)} title={task.completedAt ? t('automation.completedHint') : undefined} onClick={() => bridge && void perform(() => bridge.setAutomationEnabled(task.id, !task.enabled))}>{t(task.enabled ? 'automation.pause' : 'automation.resume')}</button><button type="button" className="pd-automation-button is-quiet pd-automation-card-history" onClick={() => showHistory(task.id)}>{t('automation.history')}</button><button type="button" className="pd-icon-button" aria-label={t('automation.deleteNamed', { name: task.name })} title={t(run ? 'automation.stopBeforeDelete' : 'automation.delete')} disabled={busy || Boolean(run)} onClick={() => setDeleteTarget(task)}><Icon name="archive" width="15" height="15" /></button></footer></article>;
				})}</div>}
