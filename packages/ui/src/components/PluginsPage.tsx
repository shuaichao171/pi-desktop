...
import { pluginResourceGroups } from '../managementState';
...
function ResourceRow({ resource, canChange, busy, onToggle, onPreview }: { resource: UiPluginResource; canChange: boolean; busy: boolean; onToggle(): void; onPreview(): void }) {
	...
	async function copyPath() {
		try { await navigator.clipboard.writeText(resource.path); setCopyStatus('copied'); }
		catch (cause) { setCopyStatus('error'); setCopyError(errorMessage(cause)); }
	}
	...
	return <article className="pd-plugin-resource" data-kind={resource.kind} data-resource-path={resource.path}>
		...
		<div className="pd-plugin-resource-copy"><div><strong>{resource.name}</strong><span className="pd-plugin-tag">{t(`plugins.scope.${resource.scope}`)}</span><span className={`pd-plugin-resource-enabled${resource.enabled ? ' is-enabled' : ''}`}>{t(resource.enabled ? 'plugins.enabled' : 'plugins.disabled')}</span></div>{resource.description && <p>{resource.description}</p>}<span className={`pd-plugin-resource-status${resource.error ? ' is-error' : ''}`} role={resource.error ? 'alert' : undefined}>{resource.error || t(!resource.enabled ? 'plugins.disabled' : resource.loaded ? 'plugins.loaded' : 'plugins.notLoaded')}</span><details className="pd-plugin-resource-path"><summary>{copy.path}</summary><code>{resource.path}</code><button type="button" className="pd-plugin-button is-quiet" onClick={() => void copyPath()}>{copy.copyPath}</button>{copyStatus !== 'idle' && <span role={copyStatus === 'error' ? 'alert' : 'status'}>{copyStatus === 'error' ? `${copy.copyFailed}: ${copyError}` : copy.copied}</span>}</details></div>
...
function ResourceGroups({ resources, expanded, renderResource }: { resources: UiPluginResource[]; expanded: boolean; renderResource(resource: UiPluginResource): ReactNode }) {
	const { t, locale } = useT();
	const copy = managementCopy(locale);
	const id = useId();
	const [collapsed, setCollapsed] = useState<Partial<Record<UiPluginResourceKind, boolean>>>({});
	return <div className="pd-plugin-resource-groups">{pluginResourceGroups(resources).map((group) => {
		const forcedOpen = expanded || group.hasError;
		const open = forcedOpen || !collapsed[group.kind];
		return <section key={group.kind} className={`pd-plugin-resource-group${group.hasError ? ' has-error' : ''}`}>
			<h3><button type="button" aria-expanded={open} aria-controls={`${id}-${group.kind}`} aria-disabled={forcedOpen || undefined} onClick={() => { if (!forcedOpen) setCollapsed((old) => ({ ...old, [group.kind]: !old[group.kind] })); }}><Icon name={KIND_ICON[group.kind]} width="16" height="16" /><span>{t(`plugins.kind.${group.kind}`)}</span><span>{group.resources.length}</span>{group.hasError && <span className="pd-plugin-group-errors">{copy.errors} {group.resources.filter((resource) => resource.error).length}</span>}<Icon name={open ? 'chevronDown' : 'chevronRight'} width="14" height="14" /></button></h3>
			<div id={`${id}-${group.kind}`} hidden={!open} className="pd-plugin-resources">{group.resources.map(renderResource)}</div>
		</section>;
	})}</div>;
}
...
export function PluginsPage({ onToggleSidebar, headerControls }: { onToggleSidebar(): void; headerControls?: ReactNode }) {
	...
	async function mutate(input: UiPluginMutation, done?: () => void, inDialog = false): Promise<boolean> {
		if (!bridge || !canChange || mutationLock.current) return false;
		const token = generation.current;
		...
		try {
			...
			if (!contextCurrent(token)) return false;
			setCatalog(value); setFeedback(t(value.warnings.length ? 'plugins.doneWithWarnings' : `plugins.done.${input.action === 'update-target' ? 'update' : input.action}`)); done?.();
			return true;
		} catch (cause) { if (contextCurrent(token)) (inDialog ? setDialogError : setError)(errorMessage(cause)); return false; }
	...
	function showDetail(item: UiPluginPackage) { listScroll.current = scrollRef.current?.scrollTop ?? 0; setDetailQuery(''); setDetailKey(packageKey(item)); scrollRef.current?.scrollTo({ top: 0 }); }
	...
	const detailResources = detail && catalog ? resourcesFor(catalog, detail).filter((resource) => `${resource.name} ${resource.description} ${resource.path} ${resource.error || ''}`.toLocaleLowerCase().includes(detailQuery.trim().toLocaleLowerCase())) : [];
		...
		<div className="pd-plugins-scroll" ref={scrollRef}><div className="pd-plugins-content">
			{detail ? <>
				...
				<div className="pd-plugin-detail-actions">
					...
					{detail.source.startsWith('npm:') && <PluginUpdateControls key={`${cwd}:${detail.scope}:${detail.source}`} item={detail} cwd={cwd} canInstall={canChange && detailWritable && !busy} install={previewId => mutate({ cwd, action: 'update-target', scope: detail.scope, previewId })} />}
					{detail.installed && detailUpdatable && !detail.source.startsWith('npm:') && <button type="button" className="pd-plugin-button" onClick={() => { setDialogError(null); setConfirm({ action: 'update', item: detail }); }} disabled={!detailWritable || busy}><Icon name="refresh" width="14" height="14" />{t('plugins.update')}</button>}
			...
			{detail && catalog ? <><section className="pd-plugin-detail-section"><h2>{t('plugins.includes')}</h2><p className="pd-plugin-note">{t('plugins.resourceHint')}</p>{resourcesFor(catalog, detail).length ? <><label className="pd-plugins-search pd-plugin-detail-search"><Icon name="search" width="15" height="15" /><input type="search" value={detailQuery} onChange={(event) => setDetailQuery(event.target.value)} aria-label={t('plugins.searchInstalled')} placeholder={t('plugins.searchInstalled')} /></label><ResourceGroups key={detailKey} resources={detailResources} expanded={Boolean(detailQuery.trim())} renderResource={renderResource} />{!detailResources.length && <p className="pd-plugin-note">{t('plugins.noMatches')}</p>}</> : <div className="pd-plugin-empty is-compact">{t('plugins.noResources')}</div>}{resourcesFor(catalog, detail).some((resource) => resource.kind === 'themes') && <p className="pd-plugin-note">{t('plugins.themeHint')}</p>}</section><section className="pd-plugin-detail-section"><h2>{t('plugins.information')}</h2><dl className="pd-plugin-info"><div><dt>{t('plugins.version')}</dt><dd>{detail.version || '—'}</dd></div><div><dt>{t('plugins.source')}</dt><dd>{detail.source}</dd></div><div><dt>{t('plugins.scopeLabel')}</dt><dd>{t(`plugins.scope.${detail.scope}`)}</dd></div>{detail.path && <div><dt>{t('plugins.path')}</dt><dd>{detail.path}</dd></div>}</dl></section></> : <>
				...
				{tab === 'installed' ? <>
					...
					{!cwd ? <div className="pd-plugin-empty"><Icon name="folder" width="28" height="28" /><h2>{t('plugins.openProjectTitle')}</h2><p>{t('plugins.openProjectHint')}</p><button type="button" className="pd-plugin-button" onClick={() => void useChatStore.getState().pickWorkspace().catch((cause) => setError(errorMessage(cause)))}>{t('sidebar.openProject')}</button></div> : loading && !catalog ? <div className="pd-plugin-empty" role="status">{t('plugins.loading')}</div> : catalog && !visiblePackages.length && !standaloneResources.length ? <div className="pd-plugin-empty"><span className="pd-plugin-avatar is-large"><Icon name="plugins" width="25" height="25" /></span><h2>{t(query || scope !== 'all' || kind !== 'all' || enabledFilter !== 'all' ? 'plugins.noMatches' : 'plugins.emptyTitle')}</h2><p>{t(query || scope !== 'all' || kind !== 'all' || enabledFilter !== 'all' ? 'plugins.noMatchesHint' : 'plugins.emptyHint')}</p><button type="button" className="pd-plugin-button" onClick={() => { setTab('discover'); setQuery(''); }}>{t('plugins.browse')}<Icon name="arrowRight" width="14" height="14" /></button></div> : <>
						...
						{standaloneResources.length > 0 && <section className="pd-plugin-list-section"><div className="pd-plugin-section-heading"><h2>{t('plugins.localResources')}</h2><span>{standaloneResources.length}</span></div><ResourceGroups resources={standaloneResources} expanded={Boolean(normalizedQuery || kind !== 'all' || enabledFilter !== 'all' || scope !== 'all')} renderResource={renderResource} /></section>}
