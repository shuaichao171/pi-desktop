...
type AttachmentPolicy =
	| { kind: 'image' | 'text' | 'pdf'; mimeType: string }
	| { errorKey: 'composer.imageTypes' | 'composer.imageEmpty' | 'composer.imageSize' | 'composer.fileTypes' | 'composer.textSize' | 'composer.pdfEmpty' | 'composer.pdfSize' };
...
export function inspectAttachmentFile(file: Pick<File, 'name' | 'type' | 'size'>): AttachmentPolicy {
	...
	if (extension === 'pdf' || file.type === 'application/pdf') { if (file.size === 0) return { errorKey: 'composer.pdfEmpty' }; if (file.size > MAX_PDF_BYTES) return { errorKey: 'composer.pdfSize' }; return { kind: 'pdf', mimeType: 'application/pdf' }; }
